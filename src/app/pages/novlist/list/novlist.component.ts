...
export class NovlistComponent implements OnInit {
  ...
  HighlightCitationRow() {
    const editedRowId = sessionStorage.getItem("citationClickRow");
    ...
    if (editedRowId) {
      ...
      sessionStorage.removeItem("citationClickRow");
    }
  }
  ngAfterViewInit(): void {
    this.fromLogin =
      this.route.snapshot.pathFromRoot[1]?.queryParams["fromLoginPage"];
  ...
  showEditButton(item: any) {
    ...
    return (
      (this.user.rolename != "TSA User" &&
        ...
        (item.currentCitationStatusId == 1 ||
          item.currentCitationStatusId == 7) &&
        item.currentCitationStatusId != 5 &&
        this.user.id == item.createdBy) ||
      (this.user.rolename == "StaffAdmin" &&
        item.currentCitationStatusId != 5) ||
      (this.user.rolename == "AuthSigner" &&
        (item.currentCitationStatusId == 3 ||
          item.currentCitationStatusId == 8)) ||
  ...
  onCitationRowClick(id: number):void{
    sessionStorage.setItem('citationClickRow', id.toString());
