...
export class BadgelistingComponent implements OnInit {
  ...
  HighlightBadgeAccRow() {
    const editedRowId = sessionStorage.getItem("clickedBadgeAudit");
    ...
    if (editedRowId) {
      ...
      sessionStorage.removeItem("clickedBadgeAudit");
    }
  }
  onEditClick(id: number): void {
    sessionStorage.setItem("clickedBadgeAudit", id.toString());
  }
  ...
  public GetBadgeAuditByRole(id: string) {
    //this.spinner.show();
    this.badgeAuditService
      .GetBadgeAuditByRole(this.queryParam, this.user.id)
      .subscribe(
        (response: DataOutputModel) => {
          this.badgeAuditDetailsList = response.items;
          this.pageHeaders = response.paging;
          if (
            this.badgeAuditDetailsList.some((x) => x.companyStatus == "True")
          ) {
  ...
  onPageChange(newPage: number): void {
    sessionStorage.setItem("BadgeListingNumber", newPage.toString());
  ...
  searchtable() {
    ...
    sessionStorage.setItem("BadgeAccSearch", this.queryParam.searchQuery || "");
    if (this.user.rolename == "AuthSigner") {
      this.GetBadgeAuditByRole(this.user.id);
    } else {
      this.GetBadgeAuditList();
