...
export class RemedialTraininglistComponent implements OnInit {
  ...
  public GetRemedialTrainingList() {
    //this.spinner.show();
    this.remedialTrainingService
      .GetRemedialTrainingList()
      .subscribe((response: RemedialTraining[]) => {
        this.remedialTraining = response;
        // console.log(response);
        this.dtTrigger.next();
        //this.spinner.hide();
        this.HighlitRemedial();
      });
  }
  ...
  public GetRemedialTrainingListNew() {
    //this.spinner.show();
    this.remedialTrainingService
      .GetRemedialTrainingListNew(this.queryParam)
      .subscribe(
  ...
  HighlitRemedial() {
    const editedRowId = sessionStorage.getItem("editedRemedialRowId");
    ...
    if (editedRowId) {
      setTimeout(() => {
        ...
        if (row) {
          ...
          row.querySelectorAll("td").forEach((td: any) => {
            td.style.setProperty("--bs-table-accent-bg", "#ccdcfc");
          });
      ...
      sessionStorage.removeItem("editedRemedialRowId");
    }
  }
  ...
  CheckBoxChange(event, id) {
    ...
    this.remedialTrainingService
      .changeActiveFlag(event.target.checked, id)
      .subscribe();
      ...
      this.remedialTrainingService
        ...
        .subscribe((response: Response) => {
          if (response.statusText == "Fail") {
            ...
            this.toastr.error(
              "Corrective Action not deleted. It is attached to a citation record",
            );
          } else if (response.statusText == "Updated") {
            this.toastr.error(
              "Corrective Action deactivated since it is already used",
            );
          } else {
  ...
  saveRemedialEdited(id: number): void {
    sessionStorage.setItem("editedRemedialRowId", id.toString());
  }
}
