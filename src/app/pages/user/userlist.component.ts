...
export class UserlistComponent implements OnInit {
  ...
  editUser(id, roid) {
    sessionStorage.setItem("editedUserRowId", roid.toString());
    const method = "e";
  ...
  ActivateUser(id: any) {
    ...
    if (ans == true) {
      //this.spinner.show()
      this._userService.activateUserByid(id).subscribe(
        (response: Response) => {
          //this.spinner.hide();
  ...
  highlightEdited() {
    const editedRowId = sessionStorage.getItem("editedUserRowId");
    ...
    if (editedRowId) {
      ...
      sessionStorage.removeItem("editedUserRowId");
    }
  }
}
